      program PREMAP
c     program to convert the magcos out file to the AVG file required by MSM
...
c      INTEGER :: i
      ...
      filein = TRIM(arg)
c
C     CALL make lstar1 once as initialisation. This is needed for kp=9 due to a possible bug in irbem.
      maginput(1,1) = 10.D0
      call make_lstar1(1,4,options,0,2000,1,0.D0,450.D0,
     *      0.D0,0.D0,maginput,lm,lstar,blocal,bmin,xj,mlt)
c
c
...
c
      maginput(1,1) = kp*10
      if ( maginput(1,1) .gt. 87.) maginput(1,1) = 87.D0
c      The IRBEM acceptes KP 0-9 (0 - 87) only
      ut = iut*3600.D0 
      write(*,*) year, kp, iut, ut, trim(fileout)," ", trim(filein)
c
